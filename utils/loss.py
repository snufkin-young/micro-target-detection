...
class ComputeLossOTA:
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            # print(pxyxys.shape[0])
            max_num = 30000
            if pxyxys.shape[0] > max_num:
                pxyxys = pxyxys[:max_num, :]
                p_obj = p_obj[:max_num]
                p_cls = p_cls[:max_num, :]
                from_which_layer = from_which_layer[:max_num]  # shape=(P)
                all_b = all_b[:max_num]  # shape=(P)
                all_a = all_a[:max_num]  # shape=(P)
                all_gj = all_gj[:max_num]  # shape=(P)
                all_gi = all_gi[:max_num]  # shape=(P)
                all_anch = all_anch[:max_num, :]  # shape=(P, 2)
